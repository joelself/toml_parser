...
impl<'a> Parser<'a> {
  ...
  method!(string<Parser<'a>, &'a str, Value># self:
    alt!(
      complete!(call_rc!(self.ml_literal_string))  => {|ml| Value::String(ml, StrType::MLLiteral)}  |
      complete!(call_rc!(self.ml_basic_string))    => {|mb| Value::String(mb, StrType::MLBasic)}  |
      complete!(call_rc!(self.basic_string))       => {|b| Value::String(b, StrType::Basic)}    |
      complete!(call_rc!(self.literal_string))     => {|l| Value::String(l, StrType::Literal)}
    )
  );
  ...
  method!(time<Parser<'a>, &'a str, Time># self:
    chain!(
      hour: re_find!("^[0-9]{2}")   ~
            tag_s!(":")             ~
   ...
   fraction: complete!(call_rc!(self.fractional)) ? ,
      ||{
        Time{
  ...
  method!(time_offset<Parser<'a>, &'a str, TimeOffset># self:
    alt!(
      complete!(tag_s!("Z"))                       => {|_|       TimeOffset::Z} |
      complete!(call_rc!(self.time_offset_amount))  => {|offset|  TimeOffset::Time(offset)}
    )
  );
  ...
  method!(date_time<Parser<'a>, &'a str, DateTime># self:
    chain!(
     date: call_rc!(self.full_date)  ~
           tag_s!("T")~
     time: call_rc!(self.time)       ~
   offset: call_rc!(self.time_offset),
        ||{
        DateTime{
  ...
  method!(pub key<Parser<'a>, &'a str, &'a str># self: alt!(complete!(call_rc!(self.quoted_key)) | complete!(call_rc!(self.unquoted_key))));

  method!(keyval_sep<Parser<'a>, &'a str, WSSep># self:
    chain!(
      ws1: call_rc!(self.ws) ~
           tag_s!("=")      ~
      ws2: call_rc!(self.ws) ,
  ...
  method!(pub val<Parser<'a>, &'a str, Value># self:
    alt!(
      complete!(call_rc!(self.array))        => {|arr|   Value::Array(Box::new(arr))}      |
      complete!(call_rc!(self.inline_table)) => {|it|    Value::InlineTable(Box::new(it))} |
      complete!(call_rc!(self.date_time))    => {|dt|    Value::DateTime(dt)}              |
      complete!(call_rc!(self.float))        => {|flt|   Value::Float(flt)}                |
      complete!(call_rc!(self.integer))      => {|int|   Value::Integer(int)}              |
      complete!(call_rc!(self.boolean))      => {|b|     Value::Boolean(b)}                |
      complete!(call_rc!(self.string))       => {|s|     s}
    )
  );

  method!(pub keyval<Parser<'a>, &'a str, KeyVal># self:
    chain!(
      key: call_rc!(self.key)        ~
       ws: call_rc!(self.keyval_sep) ~
